...
/**
 * Validation result for context file
 */
...
export class ContextFileParser {
	 ...
	 */
	async parseContextFile(filepath: string): Promise<ParsedContextFile> {
	 ...
	 */
	validateContextFile(parsedFile: ParsedContextFile): ContextFileValidationResult {
		const errors: string[] = []
	 ...
	 */
	private extractMetadataAndContent(content: string): { metadata: ContextFileMetadata; content: string } {
	 ...
	 */
	private parseYamlMetadata(yamlText: string): ContextFileMetadata {
		const metadata: Partial<ContextFileMetadata> = {}
			...
			switch (key) {
				...
				case "Trigger Reason": {
					...
					if (["manual", "automatic", "aggressive"].includes(reason)) {
						metadata.triggerReason = reason as "manual" | "automatic" | "aggressive"
					}
					break
				}
			}
		...
		if (!metadata.taskId) throw new Error("Missing Task ID in metadata")
		if (!metadata.timestamp) throw new Error("Missing Created timestamp in metadata")
		if (!metadata.contextSize) throw new Error("Missing Context Size in metadata")
		if (!metadata.triggerReason) throw new Error("Missing Trigger Reason in metadata")

		return metadata as ContextFileMetadata
	}

	/**
	 * Parse messages from markdown content
	 */
	private parseMessagesFromContent(content: string): ApiMessage[] {
	 ...
	 */
	async getFileStats(filepath: string): Promise<{ size: number; modified: number }> {
		try {
			const stats = await fs.stat(filepath)
			return {
				size: stats.size,
