...
import { ContextFileParser } from "./context-file-parser";
...
export class ContextLoader extends EventEmitter {
     ...
     */
    async loadContextFile(filepath, originalMetadata, options = {}) {
        try {
            ...
            const tokenComparison = this.createTokenComparison(originalMetadata.contextSize, editedTokens, options.contextWindow || ANTHROPIC_DEFAULT_MAX_TOKENS);
     ...
     */
    async countTokensForMessages(messages) {
        let totalTokens = 0;
     ...
     */
    createTokenComparison(originalTokens, editedTokens, contextWindow) {
     ...
     */
    validateContextLimits(tokenComparison, options) {
        const warnings = [];
        ...
        if (tokenComparison.percentageChange > 50) {
            warnings.push(`Context size increased by ${Math.round(tokenComparison.percentageChange)}% compared to original`);
        }
     ...
     */
    getTokenUsageSummary(tokenComparison) {
