 ...
 */
export const KeyboardNavigationProvider = ({ children, enabled = true, wrapNavigation = true, shortcuts = DEFAULT_SHORTCUTS, onShortcut, }) => {
    const [focusableElements, setFocusableElements] = useState([]);
    const [currentFocus, setCurrentFocus] = useState(null);
    const [focusedIndex, setFocusedIndex] = useState(-1);
    const _containerRef = useRef(null);
    // Register a focusable element
    const registerFocusable = useCallback((element, options = {}) => {
    ...
    const focusNext = useCallback((group) => {
    ...
    const focusPrevious = useCallback((group) => {
        const elements = group ? focusableElements.filter((fe) => fe.options.group === group) : focusableElements;
        if (elements.length === 0)
            return;
        const currentIndex = elements.findIndex((fe) => fe.element === currentFocus);
    ...
    const focusFirst = useCallback((group) => {
        const elements = group ? focusableElements.filter((fe) => fe.options.group === group) : focusableElements;
        if (elements.length > 0) {
            elements[0].element.focus();
        }
    }, [focusableElements]);
    const focusLast = useCallback((group) => {
        const elements = group ? focusableElements.filter((fe) => fe.options.group === group) : focusableElements;
        if (elements.length > 0) {
    ...
    return (_jsx(KeyboardNavigationContext.Provider, { value: contextValue, children: _jsx("div", { className: "keyboard-navigation-container", children: children }) }));
};
/**
...
export const KeyboardButton = ({ focusableOptions = {}, shortcut, showShortcut = false, children, ...props }) => {
    ...
    return (_jsxs("button", { ref: buttonRef, ...props, children: [children, showShortcut && shortcut && _jsx("span", { className: "ml-2 text-xs opacity-60", children: shortcut })] }));
};
export const KeyboardNavigationHelp = ({ shortcuts = DEFAULT_SHORTCUTS, className = "", }) => {
    const [isVisible, setIsVisible] = useState(false);
    const toggleHelp = () => setIsVisible(!isVisible);
    if (!isVisible) {
        return (_jsx("button", { onClick: toggleHelp, className: "fixed bottom-4 right-4 p-2 bg-blue-500 text-white rounded-full shadow-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300", "aria-label": "Show keyboard shortcuts", children: _jsx("span", { className: "text-sm", children: "\u2328\uFE0F" }) }));
    }
    return (_jsx("div", { className: `fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 ${className}`, children: _jsx("div", { className: "bg-white rounded-lg shadow-xl max-w-md w-full mx-4 max-h-96 overflow-y-auto", children: _jsxs("div", { className: "p-6", children: [_jsxs("div", { className: "flex justify-between items-center mb-4", children: [_jsx("h2", { className: "text-lg font-semibold", children: "Keyboard Shortcuts" }), _jsx("button", { onClick: toggleHelp, className: "text-gray-500 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-300", "aria-label": "Close keyboard shortcuts", children: "\u00D7" })] }), _jsx("div", { className: "space-y-2", children: Object.entries(shortcuts).map(([key, action]) => (_jsxs("div", { className: "flex justify-between items-center py-2 border-b border-gray-100", children: [_jsx("kbd", { className: "px-2 py-1 bg-gray-100 border border-gray-300 rounded text-sm font-mono", children: key }), _jsx("span", { className: "text-sm text-gray-600 capitalize", children: action.replace(/([A-Z])/g, " $1").trim() })] }, key))) }), _jsx("div", { className: "mt-4 text-xs text-gray-500", children: _jsx("p", { children: "These shortcuts work throughout the interface. Press Escape to close this help." }) })] }) }) }));
};
export const SkipLinks = ({ links }) => {
    return (_jsx("div", { className: "sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 z-50 bg-white p-4 rounded shadow-lg border", children: links.map((link, index) => (_jsx("a", { href: link.href, className: "block py-2 text-blue-600 hover:text-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-300", children: link.label }, index))) }));
};
