...
export class MarketplaceViewStateManager {
     ...
     */
    notifyStateChange(preserveTab = false) {
    ...
    async transition(transition) {
        switch (transition.type) {
            ...
            case "FETCH_COMPLETE": {
                const { items } = transition.payload;
                ...
                let newDisplayItems;
                let newDisplayOrganizationMcps;
                if (this.isFilterActive()) {
                    newDisplayItems = this.filterItems([...items], this.state.installedMetadata);
                    newDisplayOrganizationMcps = this.filterItems([...this.state.organizationMcps], this.state.installedMetadata);
                }
                else {
            ...
            case "UPDATE_FILTERS": {
                ...
                const newDisplayOrganizationMcps = this.filterItems(this.state.organizationMcps, this.state.installedMetadata);
    ...
    filterItems(items, installedMetadata) {
    ...
    async handleMessage(message) {
        ...
        if (message.type === "marketplaceButtonClicked") {
            if (message.text) {
                ...
                if (message.values?.marketplaceTab &&
...
//# sourceMappingURL=MarketplaceViewStateManager.js.map