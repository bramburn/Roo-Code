 ...
 */
export const StatusMessageSystem = ({
	messages,
	onDismiss,
	onAction,
	maxMessages = 5,
	...
	return _jsx("div", {
		className: `fixed z-50 flex ${getPositionClasses()} space-y-2 max-w-md ${className}`,
		children: groupedMessages.map((message) => {
...
const StatusMessageItem = ({ message, config, showTimestamp, onDismiss, onAction }) => {
	...
	return _jsxs("div", {
		...
		children: [
			...
			`,
				children: message.icon || config.icon,
			}),
			_jsxs("div", {
										...
										_jsxs("div", {
											...
											children: [
												...
												_jsxs("div", {
													...
													children: [Math.round(message.progress), "% complete"],
												}),
											],
										}),
									message.actions &&
										...
										_jsxs("div", {
											...
											children: [
												_jsxs("button", {
													...
													children: [isExpanded ? "Hide" : "Show", " technical details"],
												}),
												isExpanded &&
													_jsx("pre", {
														className:
															"mt-1 text-xs bg-gray-100 p-2 rounded overflow-auto max-h-32",
														children: message.technicalDetails,
													}),
											],
										}),
								],
			...
			message.priority === "critical" &&
				_jsx("div", { className: "absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full animate-pulse" }),
		],
	})
}
/**
...
export const useStatusMessages = () => {
	...
	const showLoading = (title, description, options) =>
		addMessage({ type: "loading", title, description, priority: "medium", ...options })
	const showProcessing = (title, description, options) =>
