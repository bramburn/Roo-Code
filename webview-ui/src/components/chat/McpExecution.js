...
import { mcpExecutionStatusSchema } from "@roo-code/types";
...
export const McpExecution = ({ executionId, text, serverName: initialServerName, toolName: initialToolName, isArguments = false, server, useMcpServer, alwaysAllowMcp = false, }) => {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "flex flex-row items-center justify-between gap-2 mb-1", children: [_jsxs("div", { className: "flex flex-row items-center gap-1 flex-wrap", children: [_jsx(Server, { size: 16, className: "text-vscode-descriptionForeground" }), _jsx("div", { className: "flex items-center gap-1 flex-wrap", children: serverName && _jsx("span", { className: "font-bold text-vscode-foreground", children: serverName }) })] }), _jsx("div", { className: "flex flex-row items-center justify-between gap-2 px-1", children: _jsxs("div", { className: "flex flex-row items-center gap-1", children: [status && (_jsxs("div", { className: "flex flex-row items-center gap-2 font-mono text-xs", children: [_jsx("div", { className: cn("rounded-full size-1.5", {
                                                "bg-lime-400": status.status === "started" || status.status === "completed",
                                                "bg-red-400": status.status === "error",
                                            }) }), _jsx("div", { className: cn("whitespace-nowrap", {
                                                "text-vscode-foreground": status.status === "started" || status.status === "completed",
                                                "text-vscode-errorForeground": status.status === "error",
                                            }), children: status.status === "started"
                                                ? t("execution.running")
                                                : status.status === "completed"
                                                    ? t("execution.completed")
                                                    : t("execution.error") }), status.status === "error" && "error" in status && status.error && (_jsxs("div", { className: "whitespace-nowrap", children: ["(", status.error, ")"] }))] })), responseText && responseText.length > 0 && (_jsx(Button, { variant: "ghost", size: "icon", onClick: onToggleResponseExpand, children: _jsx(ChevronDown, { className: cn("size-4 transition-transform duration-300", {
                                            "rotate-180": isResponseExpanded,
                                        }) }) }))] }) })] }), _jsxs("div", { className: "w-full bg-vscode-editor-background rounded-xs p-2", children: [useMcpServer?.type === "use_mcp_tool" && (_jsx("div", { onClick: (e) => e.stopPropagation(), children: _jsx(McpToolRow, { tool: {
                                name: useMcpServer.toolName || "",
                                description: server?.tools?.find((tool) => tool.name === useMcpServer.toolName)?.description ||
                                    "",
                                alwaysAllow: server?.tools?.find((tool) => tool.name === useMcpServer.toolName)?.alwaysAllow ||
                                    false,
                            }, serverName: useMcpServer.serverName, serverSource: server?.source, alwaysAllowMcp: alwaysAllowMcp, isInChatContext: true }) })), !useMcpServer && toolName && serverName && (_jsx("div", { onClick: (e) => e.stopPropagation(), children: _jsx(McpToolRow, { tool: {
                                name: toolName || "",
                                description: "",
                                alwaysAllow: false,
                            }, serverName: serverName, serverSource: undefined, alwaysAllowMcp: alwaysAllowMcp, isInChatContext: true }) })), (isArguments || useMcpServer?.arguments || argumentsText) && (_jsx("div", { className: cn({
                            "mt-1 pt-1": !isArguments && (useMcpServer?.type === "use_mcp_tool" || (toolName && serverName)),
                        }), children: _jsx(CodeBlock, { source: formattedArgumentsText, language: "json" }) })), _jsx(ResponseContainer, { isExpanded: isResponseExpanded, response: formattedResponseText, isJson: responseIsJson, hasArguments: !!(isArguments || useMcpServer?.arguments || argumentsText), isPartial: status ? status.status !== "completed" : false })] })] }));
};
McpExecution.displayName = "McpExecution";
const ResponseContainerInternal = ({ isExpanded, response, isJson, hasArguments, isPartial = false, }) => {
    // Only render content when expanded to prevent performance issues with large responses
    if (!isExpanded || response.length === 0) {
        return (_jsx("div", { className: cn("overflow-hidden", {
                "max-h-0": !isExpanded,
            }) }));
    }
    return (_jsx("div", { className: cn("overflow-hidden", {
        ...
        }), children: isJson ? (_jsx(CodeBlock, { source: response, language: "json" })) : (_jsx(Markdown, { markdown: response, partial: isPartial })) }));
};
const ResponseContainer = memo(ResponseContainerInternal);
//# sourceMappingURL=McpExecution.js.map