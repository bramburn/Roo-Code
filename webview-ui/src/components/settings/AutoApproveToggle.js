...
export const AutoApproveToggle = ({ onToggle, ...props }) => {
	...
	return _jsx("div", {
		className: cn("flex flex-row flex-wrap gap-2 py-2"),
		children: Object.values(autoApproveSettingsConfig).map(({ key, descriptionKey, labelKey, icon, testId }) =>
			_jsx(
				StandardTooltip,
				{
					...
					children: _jsxs(Button, {
						...
						children: [
							_jsx("span", { className: `codicon codicon-${icon} text-sm` }),
							_jsx("span", { children: t(labelKey) }),
						],
					}),
				},
				key,
