...
export const StyledPre = styled.div `
	background-color: ${CODE_BLOCK_BG_COLOR};
	max-height: ${({ windowshade, collapsedHeight }) => windowshade === "true" ? `${collapsedHeight || WINDOW_SHADE_SETTINGS.collapsedHeight}px` : "none"};
	overflow-y: auto;
	padding: 8px 3px;
	border-radius: 6px;
...
`;
const CodeBlock = memo(({ source, rawSource, language, preStyle, initialWordWrap = true, initialWindowShade = true, collapsedHeight, }) => {
    ...
    const copyButtonWrapperRef = useRef(null);
    const { showCopyFeedback, copyWithFeedback } = useCopyToClipboard();
    const { t } = useAppTranslation();
    const isMountedRef = useRef(true);
    const buttonPositionTimeoutRef = useRef(null);
    const collapseTimeout1Ref = useRef(null);
    const collapseTimeout2Ref = useRef(null);
    ...
    useEffect(() => {
        ...
        const fallback = (_jsx("pre", { style: { padding: 0, margin: 0 }, children: _jsx("code", { className: `hljs language-${currentLanguage || "txt"}`, children: source || "" }) }));
    ...
    useEffect(() => {
        ...
        if (!preElement)
            return;
        const handleScroll = () => {
            const isAtBottom = Math.abs(preElement.scrollHeight - preElement.scrollTop - preElement.clientHeight) <
    ...
    useEffect(() => {
        ...
        const handleOuterScroll = () => {
            const isAtBottom = Math.abs(scrollContainer.scrollHeight - scrollContainer.scrollTop - scrollContainer.clientHeight) <
                SCROLL_SNAP_TOLERANCE;
    ...
    const updateCodeBlockButtonPosition = useCallback((forceHide = false) => {
        ...
        if (copyWrapper) {
            ...
            if (copyRect.height > 0) {
                ...
                const buttonPadding = parseInt(buttonStyle.getPropertyValue("padding-top") || "0", 10) +
        ...
        const isPartiallyVisible = rectCodeBlock.top < scrollRect.bottom - copyButtonEdge &&
        ...
        const margin = paddingValue > 0 ? paddingValue : parseInt(computedStyle.getPropertyValue("padding-top") || "0", 10);
        ...
        if (isPartiallyVisible) {
            ...
            const topPosition = Math.max(scrollRect.top + margin, Math.min(rectCodeBlock.bottom - wrapperHeight - margin, rectCodeBlock.top + margin));
    ...
    useEffect(() => {
        ...
        const getScrollContainer = () => {
            return document.querySelector('[data-virtuoso-scroller="true"]');
        };
        ...
        const handleWheel = (e) => {
            ...
            const isAtVeryBottom = Math.abs(preRef.current.scrollHeight - preRef.current.scrollTop - preRef.current.clientHeight) < 1;
    ...
    return (_jsxs(CodeBlockContainer, { ref: codeBlockRef, children: [_jsx(MemoizedStyledPre, { preRef: preRef, preStyle: preStyle, wordWrap: wordWrap, windowShade: windowShade, collapsedHeight: collapsedHeight, highlightedCode: highlightedCode, updateCodeBlockButtonPosition: updateCodeBlockButtonPosition }), !isSelecting && (_jsxs(CodeBlockButtonWrapper, { ref: copyButtonWrapperRef, onMouseOver: () => updateCodeBlockButtonPosition(), style: { gap: 0 }, children: [showCollapseButton && (_jsx(StandardTooltip, { content: t(`chat:codeblock.tooltips.${windowShade ? "expand" : "collapse"}`), side: "top", children: _jsx(CodeBlockButton, { onClick: () => {
                            ...
                            }, children: windowShade ? _jsx(ChevronDown, { size: 16 }) : _jsx(ChevronUp, { size: 16 }) }) })), _jsx(StandardTooltip, { content: t("chat:codeblock.tooltips.copy_code"), side: "top", children: _jsx(CodeBlockButton, { onClick: handleCopy, children: showCopyFeedback ? _jsx(Check, { size: 16 }) : _jsx(Copy, { size: 16 }) }) })] }))] }));
});
// Memoized content component to prevent unnecessary re-renders of highlighted code
const MemoizedCodeContent = memo(({ children }) => _jsx(_Fragment, { children: children }));
// Memoized StyledPre component
const MemoizedStyledPre = memo(({ preRef, preStyle, wordWrap, windowShade, collapsedHeight, highlightedCode, updateCodeBlockButtonPosition, }) => (_jsx(StyledPre, { ref: preRef, preStyle: preStyle, wordwrap: wordWrap ? "true" : "false", windowshade: windowShade ? "true" : "false", collapsedHeight: collapsedHeight, onMouseDown: () => updateCodeBlockButtonPosition(true), onMouseUp: () => updateCodeBlockButtonPosition(false), children: _jsx(MemoizedCodeContent, { children: highlightedCode }) })));
