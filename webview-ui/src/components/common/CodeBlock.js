...
const CodeBlock = memo(
	({ source, rawSource, language, preStyle, initialWordWrap = true, initialWindowShade = true, collapsedHeight }) => {
		...
		useEffect(() => {
			...
			const fallback = _jsx("pre", {
				style: { padding: 0, margin: 0 },
				children: _jsx("code", {
					className: `hljs language-${currentLanguage || "txt"}`,
					children: source || "",
				}),
			})
		...
		return _jsxs(CodeBlockContainer, {
			ref: codeBlockRef,
			children: [
				_jsx(MemoizedStyledPre, {
					preRef: preRef,
					preStyle: preStyle,
					wordWrap: wordWrap,
					...
					_jsxs(CodeBlockButtonWrapper, {
						...
						children: [
							...
							_jsx(StandardTooltip, {
								...
								children: _jsx(CodeBlockButton, {
									onClick: handleCopy,
									children: showCopyFeedback ? _jsx(Check, { size: 16 }) : _jsx(Copy, { size: 16 }),
								}),
							}),
						],
					}),
...
const MemoizedStyledPre = memo(
	({ preRef, preStyle, wordWrap, windowShade, collapsedHeight, highlightedCode, updateCodeBlockButtonPosition }) =>
		_jsx(StyledPre, {
			ref: preRef,
			preStyle: preStyle,
			...
			children: _jsx(MemoizedCodeContent, { children: highlightedCode }),
		}),
)
