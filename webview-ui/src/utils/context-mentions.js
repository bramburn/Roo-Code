...
export function insertMention(text, position, value, isSlashCommand = false) {
...
(function (ContextMenuOptionType) {
    ContextMenuOptionType["OpenedFile"] = "openedFile";
    ...
    ContextMenuOptionType["Mode"] = "mode";
    ContextMenuOptionType["Command"] = "command";
    ContextMenuOptionType["SectionHeader"] = "sectionHeader";
})(ContextMenuOptionType || (ContextMenuOptionType = {}));
export function getContextMenuOptions(query, selectedType = null, queryItems, dynamicSearchResults = [], modes, commands) {
    ...
    if (/^[a-f0-9]{7,40}$/i.test(lowerQuery)) {
        const exactMatches = queryItems.filter((item) => item.type === ContextMenuOptionType.Git && item.value?.toLowerCase() === lowerQuery);
    ...
    const deduped = allItems.filter((item) => {
        ...
        let key = "";
        if (item.type === ContextMenuOptionType.File ||
