 ...
 */
export async function applySourceMapsToStack(stack) {
	if (!stack) {
 ...
 */
export async function applySourceMapsToComponentStack(componentStack) {
	if (!componentStack) {
 ...
 */
export function enhanceErrorWithSourceMaps(error, componentStack) {
 ...
 */
export async function parseStackTrace(stack) {
	if (!stack) return []
	try {
		// Create a temporary Error object with the provided stack
		const tempError = new Error()
