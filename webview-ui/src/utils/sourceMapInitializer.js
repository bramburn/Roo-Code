 ...
 */
export function exposeSourceMapsForDebugging() {
    if (process.env.NODE_ENV !== "production") {
        return;
    }
    try {
        ...
        ;
        window.__applySourceMaps = async (error) => {
        ...
        window.__testSourceMaps = () => {
            try {
                ...
                if (e instanceof Error) {
                    ...
                    window.__applySourceMaps(e).then((enhanced) => {
        ...
        window.__checkSourceMap = async (scriptUrl) => {
            try {
                const response = await fetch(`${scriptUrl}.map`);
                if (response.ok) {
                    const sourceMap = await response.json();
                    const originalFileName = sourceMap.sources && sourceMap.sources.length > 0 ? sourceMap.sources[0] : "unknown";
...
//# sourceMappingURL=sourceMapInitializer.js.map