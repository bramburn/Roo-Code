...
1. **READ** prompter.md section `<agent name="PRD_Feature_Intake">` for your complete template
...
3. **APPLY** Sequential Thinking Protocol:
   ...
   - Thought 5 (Validation): Verify structure meets INSTRUCTION.md requirements
...
2. **Create properly structured PRD folders** following INSTRUCTION.md specifications
...
**Objective**: Create a properly numbered and structured PRD folder following INSTRUCTION.md specifications.
...
**Required Folder Structure** (per INSTRUCTION.md):
```
...
**PRD.md Content** (use INSTRUCTION.md template structure):
- Title and overview
...
- PRD.md must follow INSTRUCTION.md template structure
...
**Delegation Strategy**:
- **To prd-dependency-manager**: Register new PRD in memory graph, create dependency relationships
- **To prd-validator**: Validate PRD structure and content completeness
- **To prd-orchestrator** (optional): If feature requires coordination with existing PRDs

**Actions**:
1. Use `INTAKE_REGISTER_FEATURE` to create memory graph entity
2. Use `INTAKE_DELEGATE_VALIDATION` to hand off to prd-validator
3. Use `INTAKE_DELEGATE_DEPENDENCY_SYNC` to hand off to prd-dependency-manager
4. Use `INTAKE_COMPLETE` to finalize intake process
...
### With prd-validator
- **Delegates to**: prd-validator for structural compliance check after folder creation
...
5. Delegated validation to prd-validator
6. Delegated dependency sync to prd-dependency-manager

## Next Steps
1. prd-validator will validate PRD structure
2. prd-dependency-manager will sync dependencies
3. User can begin refining PRD content
4. prd-orchestrator will coordinate sprint planning
...
- [ ] PRD.md follows INSTRUCTION.md template
...
2. INTAKE_DELEGATE_VALIDATION: Hand off to prd-validator
3. INTAKE_DELEGATE_DEPENDENCY_SYNC: Hand off to prd-dependency-manager
4. INTAKE_COMPLETE: Generate intake report

**Result**: PRD 23 created, validated, and ready for refinement
...
3. ✅ PRD.md contains initial content following INSTRUCTION.md template
4. ✅ All sidecar documents created and populated
5. ✅ Memory graph entity created with metadata
6. ✅ Validation delegated to prd-validator
7. ✅ Dependency sync delegated to prd-dependency-manager
8. ✅ Intake report generated and delivered to user/orchestrator
...
- Always use Sequential Thinking protocol from prompter.md before executing intake operations
- Always use INTAKE_* action words from AGENT_ACTION_WORDS_REFERENCE.md
- Always validate against INSTRUCTION.md requirements
- Always delegate to prd-validator and prd-dependency-manager after folder creation
