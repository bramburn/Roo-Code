...
<instructions>
    ...
    <instruction>Each PRD folder must include these required sidecar documents alongside PRD.md and README.md: CHANGELOG.md, dependencies.md, testing-strategy.md, rollback-plan.md, a sub-sprints/ directory (containing sub-sprint CANVAS docs), and a tasklists/ directory (containing tasklist_sprint_XX.md files). Skeletons may be minimal but must reference relevant PRD.md sections.</instruction>
    <instruction>In each PRD folder's README.md, add a "Quick API Links" section that directly links to the PRD's Backend section and lists key endpoints for rapid navigation when applicable.</instruction>
...
<note>
    ...
    <note>
    - **Sidecar Document Structure:** Each PRD requires a complete documentation package with supporting files.
    - **CHANGELOG.md:** Track version history and notable changes for each PRD iteration.
    - **dependencies.md:** List all technical prerequisites, external services, and system requirements.
    - **testing-strategy.md:** Document test coverage plans, test types, and validation approaches.
    - **rollback-plan.md:** Outline step-by-step procedures to revert changes if deployment fails.
    - **sub-sprints/:** Directory containing individual CANVAS documents for each sub-sprint breakdown.
    - **tasklists/:** Directory containing tasklist_sprint_XX.md files with detailed task breakdowns.
    - **Cross-references:** All sidecar documents must include links back to relevant PRD.md sections.
</note>
    <note>
...
- **Summary:** This phase builds on the core token auto-refresh capability by adding a proactive, scheduled process to refresh tokens _before_ they expire. It also introduces robust error handling and enhanced CLI visibility to alert administrators when a credential has become permanently invalid and requires manual intervention.
...
### Example: Complete PRD Sidecar Documentation Structure

The following demonstrates the complete documentation package for **PRD 1: Foundational MS Graph Token Auto-Refresh**:

#### README.md (Quick API Links Section)

```markdown
# Foundational MS Graph Token Auto-Refresh

## Quick API Links

### Backend API Endpoints

- **Token Refresh**: `outlook_integration/graph_api_client.py:_refresh_access_token()` - Core token refresh logic
- **Authentication Check**: `outlook_integration/graph_api_client.py:is_token_expired()` - Token validation
- **Credential Storage**: `outlook_integration/db_models.py:UserCredential` - Database model
- **CLI Commands**:
    - `email-analysis-cli login graph-api` - Interactive authentication
    - `email-analysis-cli ingest graph-api` - Data ingestion with auto-refresh
```

#### CHANGELOG.md

```markdown
# Changelog: Foundational MS Graph Token Auto-Refresh

## [1.0.0] - 2024-01-15

### Added

- Core MSAL-based token refresh mechanism in `GraphAPIClient`
...
- Support for `refresh_token` grant flow

### Changed

- Enhanced error handling for authentication failures
- Updated CLI to work seamlessly with auto-refresh
- Modified credential storage to track refresh timestamps
```

#### dependencies.md

```markdown
# Dependencies: Foundational MS Graph Token Auto-Refresh

## External Libraries

- **msal >= 1.28.0** - Microsoft Authentication Library for Python
- **sqlalchemy** - ORM for database operations (existing)
...
- Valid Azure AD client credentials
- Existing `UserCredential` database table with `refresh_token` column
...
```markdown
# Testing Strategy: Foundational MS Graph Token Auto-Refresh

## Test Coverage Areas

### Unit Tests

- **Token Refresh Logic**: Test `_refresh_access_token()` method with various scenarios
- **Token Expiration**: Validate `is_token_expired()` edge cases
- **Error Handling**: Test MSAL exception handling and logging

### Integration Tests

- **End-to-End Refresh**: Full workflow from expired token to successful refresh
- **Database Persistence**: Verify updated tokens are correctly saved
- **CLI Integration**: Test `ingest graph-api` with expired tokens

### Manual Testing

- **Real OAuth Flow**: Test with actual Azure AD credentials
- **Long-Running Jobs**: Verify background processes handle refresh correctly
- **Error Recovery**: Test manual re-authentication after permanent failures

## Test Environment Setup

- Mock MSAL responses for unit tests
- Test database with sample credentials
- Azure AD test application for integration tests
```

#### rollback-plan.md

```markdown
# Rollback Plan: Foundational MS Graph Token Auto-Refresh

## Rollback Scenarios

### Scenario 1: Token Refresh Logic Fails

**Symptoms**: Authentication errors, failed data ingestion
**Rollback Steps**:

1. Disable auto-refresh in configuration
2. Revert `graph_api_client.py` to previous version
3. Clear any partially refreshed tokens from database
4. Require manual re-authentication for all users

### Scenario 2: Database Schema Issues

**Symptoms**: Database errors, credential loading failures
**Rollback Steps**:

1. Stop application services
2. Restore database from pre-deployment backup
3. Verify `UserCredential` table structure matches expectations
4. Restart services with original code

### Scenario 3: MSAL Library Compatibility Issues

**Symptoms**: Import errors, runtime exceptions
**Rollback Steps**:

1. Downgrade MSAL library to previous version
2. Clear Python package cache
...
2. Verify data ingestion completes successfully
3. Check error logs for authentication issues
4. Confirm database operations complete normally
```

#### sub-sprints/ (Directory Structure)

```
sub-sprints/
├── Sub-Sprint-1-MSAL-Installation.md
├── Sub-Sprint-2-Refresh-Logic-Implementation.md
├── Sub-Sprint-3-Integration-Testing.md
└── Sub-Sprint-4-CLI-Validation.md
```

#### tasklists/ (Directory Structure)
...
- **End Date:** 2025-09-05
    ...
    <example>
        <user-request>
            Create a mobile fitness tracking app with social features.
        </user-request>
        <model-response>
            ### PRD 1: Foundational MVP - Core Fitness Tracking

            **1. Title &amp; Overview**
            - **Project:** FitTrack Mobile - MVP
            - **Summary:** A cross-platform mobile app for tracking workouts, nutrition, and progress with basic social sharing capabilities.

            **2. Goals &amp; Success Metrics**
            - **Business Objectives:** Acquire 10,000 active users in first 6 months, validate freemium model conversion rates.
            - **User Success Metrics:** 70% of users complete their first week tracking, average session duration > 5 minutes.

            **3. User Personas**
            - **Fitness Enthusiast:** Regular gym-goers wanting to track progress and share achievements.
            - **Health Beginner:** Users starting their fitness journey needing guidance and motivation.

            **4. Requirements Breakdown**
            | Phase          | Sprint   | User Story                                               | Acceptance Criteria                                                                         | Duration |
            |----------------|----------|----------------------------------------------------------|---------------------------------------------------------------------------------------------|----------|
            | Phase 1: MVP   | Sprint 1 | As a Fitness Enthusiast, I want to log my workouts.      | 1. Can create custom workout entries. 2. Exercise library with 50+ common exercises. 3. Sets, reps, weight tracking. | 3 weeks  |
            | Phase 1: MVP   | Sprint 2 | As a Health Beginner, I want to track my daily nutrition. | 1. Food database with 10,000+ items. 2. Barcode scanning functionality. 3. Calorie and macro tracking. | 3 weeks  |
            | Phase 1: MVP   | Sprint 3 | As a user, I want to see my progress over time.          | 1. Dashboard with key metrics display. 2. Progress charts for weight, strength, and measurements. 3. Goal setting and tracking. | 3 weeks  |

            **5. Complete Documentation Package Example**

            Each PRD includes these sidecar documents:

            #### README.md - Quick API Links
            ```markdown
            ## Quick API Links
            ### Mobile App Components
            - **Workout Service**: `src/services/WorkoutService.ts` - Core workout tracking logic
            - **Nutrition API**: `src/api/NutritionAPI.ts` - Food database integration
            - **Social Features**: `src/components/Social/` - Friend and challenge functionality
            - **Premium Features**: `src/features/premium/` - Subscription and advanced analytics
            ```

            #### CHANGELOG.md
            ```markdown
            # Changelog: FitTrack Mobile MVP
            ## [1.0.0] - Launch
            ### Added
            - Core workout tracking with exercise library
            ...
            #### testing-strategy.md
            ```markdown
            # Testing Strategy: FitTrack Mobile
            ## Test Types
            ### Unit Tests (Jest + React Native Testing Library)
            - Workout calculation functions
            ...
            #### rollback-plan.md
            ```markdown
            # Rollback Plan: FitTrack Mobile
            ## Hot Rollback Scenarios
            ### Critical Bug in Production
            1. Emergency patch release through app stores
            2. Server-side feature flags to disable problematic features
            3. Database transaction rollback for data corruption issues
            ```

            #### sub-sprints/ Directory
            - `Sub-Sprint-Workout-UX-Design.md`
            - `Sub-Sprint-Exercise-Database.md`
            - `Sub-Sprint-Workout-Creation.md`
