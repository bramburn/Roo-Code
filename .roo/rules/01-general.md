...
1. **Project Structure**: This is a mono-repo with Legacy Toucan (read-only) and Finch Project (active development)
2. **Repository Boundaries**: Only work within `finch/` directory - never modify legacy files
...
- **ACTIVE DEVELOPMENT**: Only work within `finch/` directory
- **READ-ONLY LEGACY**: Never modify files outside `finch/` unless explicitly instructed for migration
